...
TRACE_EVENT(rseq_ip_fixup,
	...
	TP_fast_assign(
		...
		__entry->regs_evcount = regs_evcount;
		__entry->kevcount = kevcount;
		__entry->ret = ret;
	),

	TP_printk("regs_ip=%p post_commit_ip=%p abort_ip=%p regs_evcount=%u kevcount=%u ret=%d",
		__entry->regs_ip, __entry->post_commit_ip, __entry->abort_ip,
		__entry->regs_evcount, __entry->kevcount, __entry->ret)
);
