 ...
 */
SYSCALL_DEFINE4(thread_local_abi, void *, _tlabi, uint32_t, tlabi_nr,
		uint32_t, feature_mask, int, flags)
{
	...
	if (feature_mask & TLABI_FEATURE_CPU_ID) {
		if (!tlabi_cpu_id_feature_available())
			return -ENOSYS;
	}
	if (tlabi) {
		if (current->tlabi) {
			 ...
			 */
			if (unlikely(!IS_ALIGNED((unsigned long)tlabi,
					TLABI_MIN_ALIGNMENT)))
				return -EINVAL;
	...
	/*
	 * If the CPU_ID feature is requested, ensure the cpu_id field
	 * is updated before returning to user-space.
	 */
